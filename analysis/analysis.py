...
secret_json = os.path.dirname(os.path.abspath(__file__)) + "/../PRIVATE/secret.json"
with open(secret_json) as configuration_json:
    configuration = json.load(configuration_json)

pp = pprint.PrettyPrinter(indent=4)
allinfos_url = configuration["server_ip"] + "/allPlacesInfos"
allinfos = json.loads(urllib.urlopen(allinfos_url).read())
allcaptors = []
base = datetime.datetime.today()

opening_hours_url = configuration["server_ip"] + "/sensor/getAll?s=" + configuration["html_token"]
...
for captor in allinfos:
    ...
    url = configuration["server_ip"] + "/measurements/places?ids=" + str(captor["id"]) + "&types=wifi"
